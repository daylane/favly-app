...
export class ProdutoDrawerComponent implements OnInit, OnDestroy {
  ...
  onSalvar(): void {
    ...
    const request$ = this.produto
      ? this.produtoService.editar(this.produto.id, { categoriaId, nome,  unidade: unidadeNumero, quantidadeMinima, marca })
      : this.produtoService.criar({ grupoId, categoriaId, nome,  unidade: unidadeNumero, quantidadeMinima, marca });
